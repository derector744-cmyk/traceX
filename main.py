...
def _visible_investigations(request: Request, db: Session):
    current_user = get_current_user(request, db)
...
def landing_page(request: Request, db: Session = Depends(get_db)):
    investigations = _visible_investigations(request, db)
    ctx = common_context(request, db)
    ctx["investigations"] = investigations[:6]
    ctx["total_investigations"] = len(investigations)
    ctx["total_nodes"] = db.query(GraphNode).count()
    ctx["total_evidence"] = db.query(Evidence).count()
...
def index(request: Request, db: Session = Depends(get_db)):
    ctx = common_context(request, db)
    ctx["investigations"] = _visible_investigations(request, db)
...
@app.get("/osint")
def osint_catalog_page(request: Request, db: Session = Depends(get_db)):
    ctx = common_context(request, db)
    return templates.TemplateResponse("osint.html", ctx)
